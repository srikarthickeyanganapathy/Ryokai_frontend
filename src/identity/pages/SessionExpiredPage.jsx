  ...
  return (
    <div className="flex flex-col items-center justify-center text-center space-y-6">
      ...
      <Text variant="muted" className="max-w-xs text-[13px]">
        Your session has expired due to inactivity or invalid credentials. Please sign in again to continue working.
      </Text>

      <div className="w-full pt-4">
        <Button asChild size="lg" className="w-full">
          <Link to={`/login?returnTo=${encodeURIComponent(returnTo)}`}>Sign In Again</Link>
        </Button>
      </div>
    </div>
  )
