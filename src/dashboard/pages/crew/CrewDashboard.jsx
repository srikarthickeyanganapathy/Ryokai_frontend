...
function SprintStatus({ status }) {
  return (
    <div className="p-4 rounded-xl border border-[var(--color-border-subtle)] bg-[var(--bg-elevated)] h-full">
      <Heading level={4} className="mb-4">Current Sprint: {status.name}</Heading>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <Text variant="muted" size="xs">Days Left</Text>
        ...
        <div>
          <Text variant="muted" size="xs">Points (Completed / Total)</Text>
          <Text className="text-xl font-bold">{status.completedPoints} / {status.totalPoints}</Text>
        </div>
        <div>
          <Text variant="muted" size="xs">Status</Text>
          <Text className="text-xl font-bold text-emerald-500">{status.status}</Text>
        </div>
      </div>
        ...
        {members.map(member => (
          <div key={member.id} className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5">
              <div className={`w-2 h-2 rounded-full ${member.status === 'Online' ? 'bg-emerald-500' : member.status === 'Offline' ? 'bg-[var(--text-muted)]' : 'bg-amber-500'}`} />
        ...
        {tasks.map(task => (
          <div key={task.id} className="p-3 border border-[var(--color-border-subtle)] rounded-lg bg-[var(--bg-subtle)]">
            <div className="flex justify-between items-start mb-2">
              ...
              <Text className="text-xs font-bold text-[var(--danger)] uppercase">{task.priority}</Text>
            </div>
...
function RecentActivity({ activity }) {
  return (
    <div className="p-4 rounded-xl border border-[var(--color-border-subtle)] bg-[var(--bg-elevated)] h-full">
      ...
      <div className="space-y-3">
        {activity.map(act => (
          <div key={act.id} className="flex items-start gap-2 border-b border-[var(--border-subtle)] pb-2 last:border-0 last:pb-0">
            <Icons.zap className="w-4 h-4 text-[var(--accent)] mt-0.5 shrink-0" />
            <Text className="text-sm text-[var(--text-secondary)]">{act.text}</Text>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="p-6 max-w-[1400px] mx-auto space-y-6">
      ...
      <div className="mb-6">
        ...
        <Text variant="muted">Collaborative space for your squad.</Text>
      </div>
