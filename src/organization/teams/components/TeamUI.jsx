...
export function TeamAvatar({ name, size = 'md', className, showGlow = true }) {
  ...
  return (
    <div
      className={cn(
        'rounded-2xl flex items-center justify-center font-bold text-white shrink-0 transition-transform duration-300 group-hover:scale-105 border border-white/10',
        sizes[size] || sizes.md,
        className
      )}
      style={{
...
export function MemberAvatar({ username, size = 'md', isOnline = false, className }) {
  ...
  return (
    <div className={cn('relative inline-flex shrink-0', className)} title={nameStr}>
      <div
        className={cn(
          'rounded-xl flex items-center justify-center font-bold text-white shadow-xs border border-white/10',
          sizes[size] || sizes.md
        )}
        style={{ background: `linear-gradient(135deg, hsl(${hue} 65% 50%), hsl(${(hue + 35) % 360} 60% 38%))` }}
      >
...
export function RadialProgressRing({ progress = 0, size = 48, strokeWidth = 4, hue = 230 }) {
  ...
  return (
    <div className="relative flex items-center justify-center shrink-0" style={{ width: size, height: size }} title={`${Math.round(normalizedProgress)}%`}>
      <svg width={size} height={size} className="transform -rotate-90 drop-shadow-xs">
        ...
        <circle cx={size / 2} cy={size / 2} r={radius} stroke={`hsl(${hue} 75% 52%)`} strokeWidth={strokeWidth} fill="transparent"
          strokeDasharray={circumference} strokeDashoffset={strokeDashoffset} strokeLinecap="round"
          className="transition-all duration-1000 ease-out"
        />
      </svg>
      <div className="absolute flex flex-col items-center justify-center text-center">
        <span className="font-mono text-[10px] font-bold text-[var(--text-primary)] tabular-nums leading-none">{Math.round(normalizedProgress)}%</span>
      </div>
    </div>
  )
}
