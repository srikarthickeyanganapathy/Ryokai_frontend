...
export function CreateRoleDrawer({ roles = [], open, onOpenChange, onCreate, isLoading }) {
  ...
  const [creationMode, setCreationMode] = useState('BLANK'); // 'BLANK' | 'COPY'
    ...
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent side="right" className="sm:max-w-xs p-5 flex flex-col">
        <DrawerHeader className="mb-4 text-left p-0">
          <DrawerTitle className="text-base">Create Role</DrawerTitle>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 flex-1">
          ...
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                className={cn(
                  'px-3 py-2 text-xs font-medium rounded-md border text-center transition-all',
                  creationMode === 'BLANK'
          ...
          {/* Copy Existing Dropdown */}
          {creationMode === 'COPY' && (
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-[var(--text-secondary)]">Source Role</label>
          ...
          {/* Role Name */}
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-[var(--text-secondary)]">Role Name</label>
            <Input
              value={name}
