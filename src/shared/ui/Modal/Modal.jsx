...
export const ModalContent = forwardRef(({ className, children, ...props }, ref) => (
  <ModalPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        'fixed left-[50%] top-[50%] z-50 grid w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border border-[var(--color-border-subtle)] bg-[var(--bg-elevated)] p-6 shadow-2xl duration-300 ease-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-[0.96] data-[state=open]:zoom-in-[0.96] sm:rounded-2xl max-h-[85vh] overflow-y-auto max-sm:inset-0 max-sm:max-w-none max-sm:translate-x-0 max-sm:translate-y-0 max-sm:rounded-none max-sm:h-full max-sm:max-h-none max-sm:overflow-y-auto max-sm:pt-12',
        className
      )}
      {...props}
    >
