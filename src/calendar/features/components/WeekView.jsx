...
function WeekChip({ item, onClick }) {
  ...
  return (
    <div
      ...
      className={cn(
        "flex items-baseline gap-1.5 px-2 py-1 rounded-[6px] text-[9.5px] cursor-pointer transition-colors min-w-0 border-l-[2px]",
        isEvent 
...
function PulseColumn({ day, items, count, isSelected, onSelectDay, onItemClick }) {
  const today = isToday(day)
  return (
    <button
      onClick={() => onSelectDay(day)}
      className={cn(
        "flex flex-col border border-transparent rounded-xl px-2 py-2.5 transition-colors cursor-pointer min-w-0 text-left",
        "hover:bg-[var(--bg-hover)]",
        isSelected && "bg-[var(--accent-soft)] border-[var(--accent-border)]"
      )}
    >
      <div className="flex flex-row items-center justify-center gap-1.5 py-0.5 pb-2 mb-2 border-b border-[var(--border-subtle)] w-full">
      ...
      <div className="flex items-center justify-center gap-[3px] h-[7px] w-full mb-1">
        {pulseDots(count)}
      </div>
      <div className="flex flex-col gap-1 w-full flex-1">
        ...
        {count > 4 && (
          <div className="font-mono text-[8.5px] text-[var(--text-tertiary)] text-center py-0.5">
...
export function WeekView({ tasks = [], events = [], currentDate, isLoading, onTaskClick, onEventClick, onSelectDay, selectedDay }) {
  const days = useMemo(() => {
